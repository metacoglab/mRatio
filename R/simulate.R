...
sim_metad <- function(N_trials=100, d_prime=1, c=0, log_M=0,
  ...
  d <- tidyr::expand_grid(stimulus=0:1, response=0:1, confidence=1:(length(meta_c2_0)+1)) |>
    mutate(correct=as.integer(.data$stimulus==.data$response),
    ...
    mutate(theta=metad_pmf(first(.data$stimulus), d_prime, c,
    ...
    select('stimulus', 'response', 'correct', 'confidence', 'n', 'd_prime', 'c',
...
sim_metad_condition <- function(N_trials=100, d_prime=rep(1, 2), c=rep(0, 2), log_M=rep(0, 2),
  ...
  tibble(condition=seq_along(d_prime),
    ...
    mutate(data=purrr::pmap(list(.data$d_prime, .data$c, .data$log_M,
                                 .data$c2_0_diff, .data$c2_1_diff),
                            sim_metad, N=N_trials, summarize=summarize,
...
sim_metad_participant <- function(N_participants=100, N_trials=100,
  ...
  tidyr::expand_grid(participant=1:N_participants) |>
    mutate(d_prime=rnorm(n(), mu_d_prime, sd_d_prime),
           ...
           c2_0_diff=purrr::map(.data$participant,
                                ~ exp(rmvnorm(1, mean=mu_z_c2_0,
                                              sigma=cov_matrix(sd_z_c2_0, r_z_c2_0)))),
           c2_1_diff=purrr::map(.data$participant,
                                ~ exp(rmvnorm(1, mean=mu_z_c2_1,
                                              sigma=cov_matrix(sd_z_c2_1, r_z_c2_1)))),
           data=purrr::pmap(list(.data$d_prime, .data$c, .data$log_M,
                                 .data$c2_0_diff, .data$c2_1_diff),
                            sim_metad, N=N_trials, metac_absolute=metac_absolute,
...
sim_metad_participant_condition <-
  ...
  tidyr::expand_grid(participant=1:N_participants,
    ...
    mutate(d_prime=purrr::map_dbl(.data$condition, function(condition, d) d[,condition],
                                  mvtnorm::rmvnorm(1, mu_d_prime, sigma_d_prime)),
           c=purrr::map_dbl(.data$condition, function(condition, c) c[,condition],
                            mvtnorm::rmvnorm(1, mu_c, sigma_c)),
           log_M=purrr::map_dbl(.data$condition, function(condition, log_m) log_m[,condition],
                                mvtnorm::rmvnorm(1, mu_log_M, sigma_log_M)),
           c2_0_diff=purrr::map(.data$condition, function(condition, c2) c2[,condition],
                                exp(rmatrixnorm(mu_z_c2_0,
                                                L_sigma_z_c2_0_confidence,
                                                L_sigma_z_c2_0_condition))),
           c2_1_diff=purrr::map(.data$condition, function(condition, c2) c2[,condition],
                                exp(rmatrixnorm(mu_z_c2_1,
