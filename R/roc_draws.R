...
roc1_draws <- function(object, newdata, ..., bounds=FALSE) {
  ...
  draws <- draws |>
    ...
    select(-'.category') |>
    tidyr::pivot_wider(names_from='stimulus', values_from='.epred',
                       names_prefix='p_') |>
    rename(p_hit=.data$p_1, p_fa=.data$p_0) |>
    group_by(.data$.row, !!!syms(.cols), .data$joint_response,
  ...
  if (bounds) {
    ...
    draws <- draws |>
      bind_rows(draws |>
                  ...
                  tidyr::expand_grid(tibble(joint_response=c(0, K*2),
                                            response=c(0, 1),
                                            confidence=c(K+1, K),
                                            p_fa=c(1, 0),
                                            p_hit=c(1, 0)))) |>
...
roc2_draws <- function(object, newdata, ..., bounds=FALSE) {
  ...
  draws <- draws |>
    ...
    mutate(.epred=cumsum(.data$.epred) / sum(.data$.epred),
           .epred=ifelse(.data$response, 1-.data$.epred, .data$.epred)) |>
    filter(!(.data$response==0 & .data$confidence == 1),
           !(.data$response==1 & .data$confidence == K)) |>
    select(-'.category', -'joint_response', -'stimulus') |>
    ungroup() |>
    tidyr::pivot_wider(names_from='accuracy',
  ...
  if (bounds) {
    ...
    draws <- draws |>
      bind_rows(draws |>
                  ...
                  distinct(.data$.row, !!!syms(.cols),
                           .data$.chain, .data$.iteration, .data$.draw) |>
                  tidyr::expand_grid(tibble(response=c(0, 0, 1, 1),
